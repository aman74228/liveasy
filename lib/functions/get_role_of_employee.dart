...
Future<void> getRoleOfEmployee(String uid) async {
  final snapshot = await ref
      .child(
          'companies/${shipperIdController.companyName.value.capitalizeFirst}/members/$uid')
      .get(); // This is the path for owner's shipper ID
  print(snapshot.value);
  if (snapshot.exists) {
    shipperIdController.updateOwnerStatus(snapshot.value == 'owner');
    // After getting the owner status we are updating the role status throughout the app
  } else {
    debugPrint(
        'Error in get role of employee function'); // If there is no data exist then we are using the user's shipper id only
  }
}
